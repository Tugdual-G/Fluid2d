...
def gradient(phi, dx):
    ...
    grad_j[1:-1, 1:-1] = (grad_j[1:-1, 1:-1]*0.5+(
        grad_dj[1:-1, 1:-1]+grad_di[1:-1, 1:-1])*0.5/sqrt2)

    grad_i[1:-1, 1:-1] = (grad_i[1:-1, 1:-1]*0.5+(
        -grad_dj[1:-1, 1:-1]+grad_di[1:-1, 1:-1])*0.5/sqrt2)
...
def curl(x_i, x_j, dx):
    ...
    crl[1:-1, 1:-1] = ((x_i[1:-1, 2:]-x_i[1:-1, :-2]) -
                       (x_j[2:, 1:-1]-x_j[:-2, 1:-1]))/(2*dx)
