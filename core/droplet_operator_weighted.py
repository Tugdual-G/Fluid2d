...
def gradient_i(phi, dx):
    ...
    max_grad = np.amax(np.abs(grad_i))
    grad_i[1:-1, 1:-1] = grad_i[1:-1, 1:-1]*(1-np.abs(grad_i[1:-1, 1:-1])/max_grad)+\
        (-grad_dj[1:-1, 1:-1]+grad_di[1:-1, 1:-1])*np.abs(grad_i[1:-1, 1:-1])/(max_grad*sqrt2)
...
def gradient_j(phi, dx):
    ...
    # Projecting diagonal gradients, onto the i and i directions,
    # adding to i and j gradients and averaging:
    max_grad = np.amax(np.abs(grad_j))
    grad_j[1:-1, 1:-1] = grad_j[1:-1, 1:-1]*(1-np.abs(grad_j[1:-1, 1:-1])/max_grad)+\
        (grad_dj[1:-1, 1:-1]+grad_di[1:-1, 1:-1])*np.abs(grad_j[1:-1, 1:-1])/(max_grad*sqrt2)
