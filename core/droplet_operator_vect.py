...
def torque(dxdt, phi, density, dx, rho_l, rho_h, xi, sigma, gravity=10):
    ...
    vect = kap_j*gradient_i(phi, dx)-kap_i*gradient_j(phi, dx)

    F_i = (4*eta*phi*(phi-1)*(phi-1/2))*gradient_i(phi, dx)

    F_j = (4*eta*phi*(phi-1)*(phi-1/2))*gradient_j(phi, dx)

    dxdt += gradient_j(density, dx)*gravity + curl(F_i, F_j, dx)+vect
